<?php
	...
	class DatabaseObject_Event extends DatabaseObject
	{
		public $profile=null;
		public $images = array();
		...
		public function __construct($db)
		{
			...
			parent::__construct($db, 'events', 'event_id');
			$this->add('user_id');
			$this->add('url');
		...
		public function loadByPromotionCode($code)
		{
			
			$select = $this->_db->select();
			
			$select->from('events_profile', 'event_id')
				->where('profile_value = ?', $code)
				->where('profile_key = ?', 'promotion_code');				
			$id = $this->_db->fetchRow($select);
			
			echo "select is: ".$select;
			echo "id is: ".$id['event_id'];


			return $this->eventTestLoad($id['event_id']);		
		}
