...
<div id="productDetailRightSidePanel" class='box' style='float:right; width: 410px; padding-right:10px;'>
    ...
    <div class="productProfiles box marginTop20" style='text-align: left; width:100%;'>
         ...
         {include file="productdisplay/_productDetailsAttribute/_$orderAttribute.tpl" product=$product}
 	
	 <div class='quickOrderActions' style='float:left; width:100%;'>
	 {if isset($product.0.product_inventory_id)}
	 	{if $product.0.purchase_type=='Customizable'}
	  <a class='largeAnchorButton fullOrderDetailsColorBox' style='float:left; padding-left:10px; padding-top:10px;' href="{geturl controller='productdisplay' action='purchasedetails'}?number={$product.0.product_id}&product=products">Customize this</a>
	  {/if}
     <form method='post' action="{geturl controller='shoppingcart' action='additemtoshoppingcart'}?product=Inventory&id={$product.0.product_inventory_id}">
