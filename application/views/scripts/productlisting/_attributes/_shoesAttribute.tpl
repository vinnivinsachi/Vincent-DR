	...
	<select name='inventory[sys_shoe_metric]' class='inputShiftOne'>
		<option value='EU' {if $inventory.basic.sys_metric_type=='EU'} selected="selected"{/if}>Euro Size</option>
		<option value='US' {if $inventory.basic.sys_metric_type=='US'} selected="selected"{/if}>US Size</option>
		<option value='BR' {if $inventory.basic.sys_metric_type=='BR'} selected="selected"{/if}>British Size</option>
	</select>
	<select name="inventory[sys_shoe_size]">
		{foreach from=$sysShoeSizes item=measurement}
